...
# === 설정 ===
MODEL_NAME = "syntheticbot/ocr-qwen"
MODEL_DIR = os.path.abspath("local-models")
ARTIFACTS_PATH = f"{MODEL_DIR}/syntheticbot/ocr-qwen/"
...
# OCR QWen 모델에 최적화된 설정
tensor_parallel_size = 2
max_model_len = 2048  # OCR QWen은 더 긴 컨텍스트 지원
block_size = 2048
...
# 샘플 프롬프트 (OCR QWen 모델에 최적화된 한국어 프롬프트)
prompts = [
...
def check_and_download_model():
    ...
    if not os.path.exists(config_path):
        print(f"[안내] OCR QWen 모델 파일이 {ARTIFACTS_PATH}에 없습니다.")
        
        # 토큰 확인
        if not check_hf_token():
            print("[오류] OCR QWen 모델에 접근하려면 HuggingFace 토큰이 필요합니다.")
            return False
        
        print("HuggingFace Hub에서 OCR QWen 모델을 다운로드합니다...")
        ...
        try:
            ...
            if not os.path.exists(config_path):
                print(f"[오류] OCR QWen 모델 다운로드에 실패했습니다.")
                return False
        except Exception as e:
            print(f"[오류] OCR QWen 모델 다운로드 실패: {e}")
            return False
...
def main():
    if not check_and_download_model():
        print("[오류] OCR QWen 모델 다운로드에 실패했습니다. 프로그램을 종료합니다.")
        return

    print(f"[안내] OCR QWen 모델을 로드합니다...")
