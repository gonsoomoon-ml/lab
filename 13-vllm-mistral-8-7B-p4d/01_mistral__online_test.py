...
def test_single_request(prompt, model_name="mistralai/Mixtral-8x7B-Instruct-v0.1"):
    """단일 요청 테스트"""
    ...
    try:
        ...
        response = requests.post(
            "http://localhost:8000/v1/chat/completions",  # 포트를 8000으로 변경
            headers=headers,
            json={
                "model": model_name,
...
def check_available_models():
    """사용 가능한 모델 목록을 확인합니다."""
    try:
        response = requests.get("http://localhost:8000/v1/models")  # 포트를 8000으로 변경
