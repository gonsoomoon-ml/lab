...
def plot_1hour_detailed_comparison(actual, predicted, timestamps, output_path):
    ...
    ax1.plot(detail_time, detail_pred,
             label='Prophet Prediction (예측값)', linewidth=2, alpha=0.85,
             linestyle='--', color='#9370DB', marker='s', markersize=4,
             markeredgecolor='white', markeredgewidth=0.5)
    ...
    for i in [0, 15, 30, 45, 59]:
        if i < len(detail_actual):
            ...
            # Predicted value (purple)
            ax1.annotate(f'{detail_pred.iloc[i]:.1f}',
                        xy=(detail_time.iloc[i], detail_pred.iloc[i]),
                        xytext=(0, -15), textcoords='offset points',
                        fontsize=9, color='#9370DB', fontweight='bold',
                        ha='center',
                        bbox=dict(boxstyle='round,pad=0.3',
                                facecolor='white', edgecolor='#9370DB', alpha=0.8))
...
def main():
    print("=" * 80)
    print("Prophet 상세 시각화")
    print("=" * 80)

    # Load predictions
    print("\n데이터 로딩...")
    predictions = pd.read_csv('results/prophet/prophet_predictions.csv')

    # Load original test data for timestamps
    test_original = pd.read_csv('data/test.csv')
    timestamps = pd.to_datetime(test_original['timestamp'])

    # Prophet은 lag feature를 사용하지 않으므로 timestamp 정렬만 필요
    ...
    timestamps_aligned = timestamps.reset_index(drop=True)

    print(f"  총 {len(predictions):,}개 샘플")

    # Create 1-hour detailed view
    print("\n1시간 상세 시각화 생성...")
    output_path = 'results/prophet/prophet_1hour_detailed.png'
