...
def plot_1hour_detailed_comparison(actual, predicted, timestamps, output_path):
    ...
    ax1.plot(detail_time, detail_pred,
             label='Random Forest Prediction (예측값)', linewidth=2, alpha=0.85,
             linestyle='--', color='#228B22', marker='s', markersize=4,
             markeredgecolor='white', markeredgewidth=0.5)
...
def main():
    print("=" * 80)
    print("Random Forest 상세 시각화")
    print("=" * 80)

    # Load predictions
    print("\n데이터 로딩...")
    predictions = pd.read_csv('results/random_forest/random_forest_predictions.csv')

    # Load original test data for timestamps
    test_original = pd.read_csv('data/test.csv')
    timestamps = pd.to_datetime(test_original['timestamp'])

    # Align timestamps (1440 samples removed due to lag features)
    timestamps_aligned = timestamps.iloc[1440:1440+len(predictions)]
    ...
    print("\n1시간 상세 시각화 생성...")
    output_path = 'results/random_forest/random_forest_1hour_detailed.png'
