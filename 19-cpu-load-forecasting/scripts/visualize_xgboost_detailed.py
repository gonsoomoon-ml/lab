...
def plot_1hour_detailed_comparison(actual, predicted, timestamps, output_path):
    ...
    ax1.plot(detail_time, detail_pred,
             label='XGBoost Prediction (예측값)', linewidth=2, alpha=0.85,
             linestyle='--', color='#FF6600', marker='s', markersize=4,
             markeredgecolor='white', markeredgewidth=0.5)
    ...
    for i in [0, 15, 30, 45, 59]:
        if i < len(detail_actual):
            ...
            ax1.annotate(f'{detail_pred.iloc[i]:.1f}',
                        ...
                        ha='center',
                        bbox=dict(boxstyle='round,pad=0.3',
                                facecolor='white', edgecolor='#FF6600', alpha=0.8))
...
def main():
    print("=" * 80)
    print("XGBoost 상세 시각화")
    print("=" * 80)

    # Load predictions
    print("\n데이터 로딩...")
    predictions = pd.read_csv('results/xgboost/xgboost_predictions.csv')
    ...
    print("\n1시간 상세 시각화 생성...")
    output_path = 'results/xgboost/xgboost_1hour_detailed.png'
